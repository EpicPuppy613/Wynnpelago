...
public class WynnpelagoCommand {
    ...
    private static int executeVersionCommand(CommandContext<FabricClientCommandSource> context) {
        context.getSource().sendFeedback(WynnpelagoClient.getWPPrefix()
                .append(Component.literal("Wynnpelago v" + Wynnpelago.VERSION)));
        return 1;
    }
    ...
    private static int executeTerritoryUnlockCommand(CommandContext<FabricClientCommandSource> context) {
        ...
        if (TerritoryUnlock.unlockedTerritories.contains(territory)) {
            context.getSource().sendError(WynnpelagoClient.getWPPrefix()
                    .append(Component.literal("Territory already unlocked").withStyle(ChatFormatting.RED)));
            return 0;
            ...
            context.getSource().sendFeedback(WynnpelagoClient.getWPPrefix()
                    .append(Component.literal("Unlocked territory: " + territory).withStyle(ChatFormatting.GREEN)));
            return 1;
        }
    }
    ...
    private static int executeTerritoryLockCommand(CommandContext<FabricClientCommandSource> context) {
        ...
        if (!TerritoryUnlock.unlockedTerritories.contains(territory)) {
            ...
            context.getSource().sendError(WynnpelagoClient.getWPPrefix()
                    .append(Component.literal("That territory is always unlocked").withStyle(ChatFormatting.RED)));
            return 0;
        } else {
            TerritoryUnlock.unlockedTerritories.remove(territory);
            context.getSource().sendFeedback(WynnpelagoClient.getWPPrefix()
                    .append(Component.literal("Locked territory: " + territory).withStyle(ChatFormatting.GREEN)));
            return 1;
        }
    }
}
